...
`data/pattern_manifest.csv` maps the 16,384 GCS+S acquisition positions to `(u,v)` Hadamard coordinates and the matching positive/complementary pattern identifiers. The companion experimental records use this installed manifest.
...
## `paw_print` reference checkpoints

The companion dataset stores manuscript reference checkpoints separately from reader-generated outputs:

- `reference_results/paw_print/` — reference bucket values, measurement vectors, reconstructions, and metrics;
- `reference_figures/paw_print/` — reference PNG/PDF manuscript-style figure exports.
